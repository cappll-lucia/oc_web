...
import SolShop from '../../components/Shops/SolShop';

const getProducts = async () => {
	try {
		const response = await axios.get(`${process.env.VITE_API_URL}/products/category/1`);
...
export default async function AnteojosSol() {
	...
	return (
		<div className='sol-page'>
			<h1 className='title'>Anteojos de Sol</h1>
			<div className='warn'>
			...
			<SolShop products={products} colors={colors} brands={brands} />
		</div>
	);
}
